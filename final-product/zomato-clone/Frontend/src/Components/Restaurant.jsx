...
const Restaurant = (props) => {
    ...
    const [mobile, setMobile] = useState("9000458244");
    const [address, setAddress] = useState("MG Road , Bangalore");
  
    let getRestaurantDetails = async () => {
      let url =
        `http://localhost:3001/api/get-restaurantdetails-restoid/` + id;
      let { data } = await axios.get(url);
      setrDetails(data.restaurantId);
    };
  
    let getMenuItems = async () => {
      let url = `http://localhost:3001/api/get-menuitems-restoid/` + id;
      let { data } = await axios.get(url);
      setTotal(0);
